...
/**
     ...
     * @param _purpose the purpose of the token (coin, coupon, ...).
    ...
    function createToken(
        ...
        tokenFactory.createToken(_name, _symbol, _decimals, _purpose, _logoURL, _logoHash, _hardCap, _contractHash);
    }

    /**
     * @dev mintToken mints the specified tokens to the DAO wallet.
     *      Requires that the DAO owns the token.
     * @param _name The name of the token.
    ...
    function mintToken(
        ...
        require(myRole() >= ROLE_ADMIN, "mintToken: Must be admin to call");
        address addr;
        (addr, , , , , ,) = tokenFactory.getToken(_name);
        require(addr != address(0x0), "mintToken: The token must exist");
        TokenTemplate tkn = TokenTemplate(addr);
    ...
    function createCrowdsale(
        ...
        uint256 _acceptRatio, uint256 _giveRatio,
        uint256 _maxCap
    ) public {
